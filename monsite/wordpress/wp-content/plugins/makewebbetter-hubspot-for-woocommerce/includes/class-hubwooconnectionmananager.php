...
class HubWooConnectionMananager {
	...
	/**
	 * Import bulk products to hubspot.
	 *
	 * @param string $file Path to products csv file.
	 */
	public function import_historical_contacts( $file = '' ) {

		$endpoint = '/crm/v3/imports';

		$acces_tokens = Hubwoo::hubwoo_get_access_token();

		$headers = array(
			'Authorization: Bearer ' . $acces_tokens,
		);

		$csv_file = new CURLFile( $file, 'text/csv' );

		$config_json = '{
			"name": "Historical Contacts' . time() . '",
			"files": [
			  {
		...
		$message         = __( 'Syncing bulk contacts', 'makewebbetter-hubspot-for-woocommerce' );
		$this->create_log( $message, $endpoint, $parsed_response );
		return $parsed_response;
	}
}
